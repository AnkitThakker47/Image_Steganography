...
def test(cw):
	...
	newTop.geometry("450x250+500+300")
	container = Frame(newTop, bg='#02001c')  
	container.grid(row=0, column=0,padx=30,pady=(30,5))
	lab=Label(container,text="Enter the keywords separated by space",bg="#02001c",fg="gold")
	getword=Entry(container,width=60,relief="sunken")
	lab2=Label(container,text="Enter the nunber of images you want",bg="#02001c",fg="gold")
	getnum=Entry(container,width=25,relief="sunken")
	btnFolder=Button(container,text="Select folder to save images",bg="deepskyblue",fg="#02001c",command=select_dir,width=20)
	btn=Button(container,text="Search and save",bg="deepskyblue",fg="#02001c",command=lambda: fetchImages(getword.get(),getnum.get()),width=20)
	btnback=Button(newTop,text="Continue",bg="salmon",fg="#02001c",command=goback)
	lab.grid(pady=10,padx=10,row=0,column=0,columnspan=2)
	getword.grid(pady=10,padx=10,row=1,column=0,columnspan=2)
	lab2.grid(pady=5,row=2,column=0,padx=(10,0))
	getnum.grid(pady=5,padx=(0,10),row=2,column=1)
	btnFolder.grid(pady=20,padx=(10,2),row=3,column=0)
	btn.grid(pady=20,padx=(2,10),row=3,column=1)
	btnback.grid(pady=(0,10),padx=1,sticky=E,row=1,column=0)
